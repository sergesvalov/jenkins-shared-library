...
### Генератор пайплайнов (Новый подход)

Чтобы ошибки конфигурации отлавливались на этапе коммита и чтобы поддерживать сложные кастомные стадии ("снежинки"), теперь можно явно генерировать `Jenkinsfile` из `pipeline-config.yaml` с помощью встроенного скрипта-генератора.

1. **Валидация (Pre-commit)**: Запуская генератор локально (например, через Husky), вы валидируете `pipeline-config.yaml` по JSON Schema до коммита. Кривое описание роняет коммит, а не сборку.
2. **Аварийный люк (Escape hatches)**: Вы можете добавить блок `custom_stages` в ваш `pipeline-config.yaml`, чтобы внедрить специфические шаги в сгенерированный пайплайн. Это позволяет нестандартным проектам использовать общий шаблон без форков.
3. **Сухой прогон (Dry-run)**: Запустите генератор с флагом `--dry-run`, чтобы увидеть разницу между тем, что написано руками, и тем, что сгенерировал бы он (без перезаписи).
