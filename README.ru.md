...
Чтобы поддерживать уникальные требования проектов, сохраняя при этом стандартизацию пайплайнов, `declarativePipeline` предоставляет встроенную модель расширяемости. Вы можете либо наследовать и изменять стандартный шаблон через `custom_stages` (Аспектно-ориентированный подход), либо компоновать свой пайплайн из низкоуровневых шагов (Композиция).

#### 1. Серверная валидация и сухой прогон (Dry-Run)
В самом начале сборки Jenkins валидирует ваш `pipeline-config.yaml` по JSON Schema. Если описание содержит ошибку, сборка падает немедленно.
...
#### 2. Наследование и модификация (`custom_stages`)
...
* Без стратегии (по умолчанию) — Шаг добавится в самый конец пайплайна.

**Пример: Замена стандартной стадии и добавление новой:**
```yaml
service_name: "myapp"
stack_type: "capacitor"
custom_stages:
...
#### 3. Композиция (Пересоздание пайплайна)
Если проект сильно нестандартный и подход с `custom_stages` не спасает, вы можете полностью отказаться от `declarativePipeline` и написать обычный `Jenkinsfile` прямо в репозитории. Но вместо того, чтобы писать всё с нуля, вы строите свой кастомный процесс, используя низкоуровневые "кубики" (функции) из нашей библиотеки.
