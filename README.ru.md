...
### Серверная валидация и генератор пайплайнов (Новый подход)

Чтобы ошибки конфигурации отлавливались как можно раньше, а сложные кастомные стадии ("снежинки") можно было легко поддерживать, `declarativePipeline` теперь оснащен встроенной серверной проверкой.

Ваш `Jenkinsfile` по-прежнему остается однострочным (`declarativePipeline(agent: 'built-in')`), но под капотом Jenkins сделает следующее:

1. **Ранняя валидация (Early Validation)**: В самом начале сборки Jenkins вытаскивает генератор из папки `resources/` библиотеки и валидирует ваш `pipeline-config.yaml` по JSON Schema. Кривое описание моментально роняет сборку на первых же секундах, не дожидаясь долгих этапов.
2. **Сухой прогон (Режим VALIDATE_ONLY)**: Если вы хотите проверить, как будет выглядеть ваш пайплайн без фактической сборки и деплоя, вы можете запустить джобу в Jenkins и отметить галочку **`VALIDATE_ONLY`**. Пайплайн проверит конфиг, распечатает сгенерированный `Jenkinsfile` в логи консоли и успешно завершится.
3. **Аварийный люк (Escape hatches)**: Вы можете добавить блок `custom_stages` в ваш `pipeline-config.yaml`, чтобы внедрить специфические шаги в динамически генерируемый пайплайн. Это позволяет нестандартным проектам использовать общий шаблон без форков библиотеки.

**Пример `pipeline-config.yaml` с кастомной стадией:**
```yaml
service_name: "myapp"
stack_type: "capacitor"
custom_stages:
